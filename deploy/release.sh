...
if [[ $target == indexer || $target == both ]]; then
  ...
  # sudo resets the environment, so the variables ride on `sudo env`, not on
  # the calling shell.
  r "sudo -n env IDX=$IDX_ROOT/releases/$ts NODE_BIN=$NODE_BIN IDX_USER=$IDX_USER bash -s" <<'REMOTE' | tail -2 || on_fail "indexer npm ci"
set -euo pipefail
cd "$IDX"
...
REMOTE
  ...
  r "sudo -n env REL=$IDX_ROOT/releases/$ts NODE_BIN=$NODE_BIN IDX_USER=$IDX_USER bash -s" <<'REMOTE' || on_fail "indexer module smoke"
set -euo pipefail
