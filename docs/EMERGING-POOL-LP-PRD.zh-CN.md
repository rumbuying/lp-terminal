...
- 2026-09-19 静默降级(产品决策,修订 §4.1 的纯 FIFO 录取):录取后 `quietDemoteHours`(6h)内零成交(无 canonical swap/v4raw 事件)的池释放详细采集名额——状态 `queued`/`quiet_demoted`,补位队列中排在未录取池之后;发现账本与事件历史零删除,成本仅为每个零成交池一个 6h 观察窗。pin 池不受降级影响。动机:上线实测大部分新池零成交,纯 FIFO 会让采集名额被死池占满(运营反馈 2026-09-19)。
