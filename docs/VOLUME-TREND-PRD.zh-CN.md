...
2026-09-19 幼龄池接续：[幼龄池 PRD v2](EMERGING-POOL-LP-PRD.zh-CN.md) 在独立只读流水线中提供 `watch_candidate`（“观察条件满足”），不改变本节成熟池评分和 FR-EXEC-1 的不自动交易边界。EM-A/B/C 不创建策略；EM-D 的人工入场、保护退出和预算需另满足该 PRD 的证据与执行条件，不能由本节趋势告警隐式开启。
...
幼龄池展示扩展以 [幼龄池 PRD §8](EMERGING-POOL-LP-PRD.zh-CN.md) 为准：现有 EmergingRow 保留展示语义，独立 API 覆盖含 V4 的发现账本；不使用展示行截断列表构造 cohort。新增徽章说明证据与新鲜度，不得把“未发现指定异常”标成“安全”或“进场”。
