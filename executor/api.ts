...
const JSONH = {
  ...
  // The browser checks this on every response. A reverse-proxy mapping error
  // must fail closed before BSC credentials or commands reach Robinhood (or
  // vice versa).
...
export function startApi() {
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/v1/pnl-curve') {
        const now = Math.floor(Date.now() / 1000)
        const from = url.searchParams.get('from'), to = url.searchParams.get('to')
        const fromTs = from && /^\d+$/.test(from) ? Number(from) : now - 30 * 24 * 60 * 60
      ...
      if (req.method === 'PUT' && url.pathname.startsWith('/v1/strategies/')) {
        ...
        if (config.chainId !== EXECUTOR.chainId) return json(res, 400, { error: `strategy chain must be ${EXECUTOR.chainId}` })
        if (config.id !== id) return json(res, 400, { error: 'strategy id mismatch' })
