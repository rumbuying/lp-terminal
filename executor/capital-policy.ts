...
const routeCurrency = (token: Address) => low(token) === low(zeroAddress) ? NATIVE : token
...
export async function strategyCapitalHarvest(args: {
  ...
  const thresholdUsdg = parseUnits(args.config.capitalProtection.profitThresholdUsdg, EXECUTOR.network.settlementDecimals)
  ...
  if (excessValueQuote > 0n) {
    excessValueUsdg = low(args.config.quoteToken) === low(EXECUTOR.network.settlementToken)
      ? excessValueQuote
      : BigInt((await quoteKyber(routeCurrency(args.config.quoteToken as Address), EXECUTOR.network.settlementToken, excessValueQuote)).routeSummary.amountOut)
  }
  return planCapitalHarvest({
