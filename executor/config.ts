...
import { isStrategyChainId, strategyNetwork, type StrategyChainId } from '../src/config/networks'
...
const configuredChainId = Number(env('LP_EXECUTOR_CHAIN_ID') ?? CHAIN_ID)
if (!isStrategyChainId(configuredChainId)) throw new Error('LP_EXECUTOR_CHAIN_ID must name a configured strategy chain')
const chainId = configuredChainId as StrategyChainId
if (chainId !== CHAIN_ID) throw new Error(`LP_EXECUTOR_CHAIN_ID=${chainId} must match CHAIN build identity ${CHAIN_ID}`)
const network = strategyNetwork(chainId)

const dataDir = env('LP_EXECUTOR_DATA_DIR') ?? fileURLToPath(new URL(chainId === 56 ? './data-bnb' : './data', import.meta.url))
mkdirSync(dataDir, { recursive: true, mode: 0o700 })

const rpc = exclusiveTextSource('LP_EXECUTOR_RPC', 'LP_EXECUTOR_RPC_FILE')
const rpcUrl = rpc.value ?? network.publicRpc
...
export const EXECUTOR = {
  ...
  network,
  host,
  port: Number(env('LP_EXECUTOR_PORT') ?? (chainId === 56 ? 8791 : 8790)),
  dataDir,
  ...
  kyberChain: env('LP_EXECUTOR_KYBER_CHAIN') ?? network.slug,
