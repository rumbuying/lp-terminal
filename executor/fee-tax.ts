...
export async function planFeeTax(args: {
  ...
  const rewardToken = args.rewardToken ?? WRAPPED_NATIVE
  ...
  const valueInUsdg = async (token: Address, amount: bigint) => {
    ...
    if (low(token) === low(SETTLEMENT)) return amount
    return (await args.quote(token, SETTLEMENT, amount)).amountOut
  }
  ...
  const threshold = EXECUTOR.chainId === 4663 ? FEE_TAX_THRESHOLD_USDG : 10n ** BigInt(EXECUTOR.network.settlementDecimals)
  if (totalIncomeUsdg <= threshold) {
  ...
  const source = preferred(SETTLEMENT) ?? preferred(WRAPPED_NATIVE) ?? largest
  ...
  const rawTax = low(sourceToken) === low(SETTLEMENT)
    ? targetUsdg
  ...
  for (const [token, amount] of [[args.token0, tax0], [args.token1, tax1], [rewardToken, rewardTax]] as const) {
    ...
    if (low(token) === low(SETTLEMENT)) {
      retainedUsdg += amount
      continue
    }
    const quoted = await args.quote(token, SETTLEMENT, amount)
    swaps.push({
      purpose: 'fee_tax',
      tokenIn: token,
      tokenOut: SETTLEMENT,
