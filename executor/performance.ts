...
function cycleIncomeTaxQuote(cycle: CycleRow, rows: LedgerRow[], config: StrategyConfig, price: CyclePrice): bigint {
  ...
  try {
    return swappedTax + quoteTurnover(BigInt(retainedRaw), ADDR.USDG, config, price.snapshot, price.sqrtPriceX96)
  } catch {
    return swappedTax
  }
}
...
async function valueGasInQuote(cycles: CycleRow[], ledger: LedgerRow[], config: StrategyConfig, openingGasWei: bigint): Promise<GasValuation> {
  ...
  try {
    totalQuote = low(config.quoteToken) === low(ADDR.WETH)
      ? totalWei
      : BigInt((await quoteWithNativeFallback(ADDR.WETH, config.quoteToken, totalWei)).routeSummary.amountOut)
  } catch (error) {
    return {
      opening: 0n,
...
async function tokenSymbol(token: Address): Promise<string> {
  if (low(token) === low(ADDR.WETH)) return 'WETH'
  const key = low(token)
...
export async function strategyPerformance(config: StrategyConfig, state: string) {
  ...
  const warnings = [
    ...
    ...(!gasValuation.error && gasValuation.total > 0n && low(config.quoteToken) !== low(ADDR.WETH) ? ['gas_quote_current_price'] : []),
