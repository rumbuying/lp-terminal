 ...
 */
export async function preflightStrategy(config: StrategyConfig, options: { rangeScale?: number; manualExecution?: true } = {}) {
  ...
  const plan = makeRebalancePlan({ config, snapshot, triggerSide, rangeScale, manualExecution: options.manualExecution })
