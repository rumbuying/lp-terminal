...
async function finalizeWithdrawal(args: {
  ...
  let gasQuote = 0n
  if (gasWei > 0n) {
    gasQuote = low(args.config.quoteToken) === low(ADDR.WETH)
      ? gasWei
      : await quoteKyber(ADDR.WETH, args.config.quoteToken, gasWei).then((quote) => BigInt(quote.routeSummary.amountOut)).catch(() => 0n)
  }
...
async function valueOutput(amount: bigint, token: Address, quoteToken: Address) {
  const quoteValue = low(token) === low(quoteToken)
    ? amount
    : low(token) === low(ADDR.USDG) && low(quoteToken) === low(ADDR.WETH)
    ? await currentUsdgValueInWeth(amount)
    : BigInt((await quoteKyber(token, quoteToken, amount)).routeSummary.amountOut)
  const usdgValue = low(token) === low(ADDR.USDG)
    ? amount
...
export async function withdrawRetainedProfit(strategyId: string, target: ProfitWithdrawalTarget): Promise<ProfitWithdrawalResult> {
  ...
  return withWalletLock(walletId, async () => {
    ...
    const settlementToken = target === 'USDG' ? ADDR.USDG : ADDR.WETH
    ...
    try {
      ...
      for (const source of sources) {
        ...
        const quote = await quoteKyber(source.token, settlementToken, source.amount)
        ...
        const spent = before[low(source.token)] - after[low(source.token)]
        const gained = after[low(settlementToken)] - before[low(settlementToken)]
      ...
      if (target === 'ETH') {
        ...
        const unwrapReceipt = await sendTracked({
          ...
          tx: { to: ADDR.WETH, data: encodeFunctionData({ abi: wethAbi, functionName: 'withdraw', args: [amountOut] }), value: 0n },
        })
        receipts.push(unwrapReceipt)
        const after = await readTokenBalances(row.config.owner, trackedTokens)
        if (before[low(ADDR.WETH)] - after[low(ADDR.WETH)] !== amountOut) throw new Error('E_UNSUPPORTED_TOKEN')
      }
...
export async function recoverRetainedProfit(jobId: string) {
  ...
  for (const swap of context.swaps) {
    ...
    const recorded = db.prepare(`SELECT 1 FROM ledger_entries WHERE strategy_id=? AND tx_hash=? AND kind='profit_withdrawal_swap_in' LIMIT 1`).get(job.config.id, receipt.transactionHash)
