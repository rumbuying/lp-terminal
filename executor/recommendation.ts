...
async function tokenUsd(address: string, candidates: RecommendationCandidate[]): Promise<number | null> {
  if (address.toLowerCase() === ADDR.STABLE.toLowerCase()) return 1
...
function profile(protocol: RecommendationProtocol, samples: CostSamples, source: RecommendationCostProfile['source']): RecommendationCostProfile {
  return {
    protocol,
...
export async function recommendations(args: { capitalUsd: number; mode: RecommendationMode; risk: RecommendationRisk; limit: number }): Promise<RecommendationResponse> {
  ...
  const scored = body.candidates.flatMap((candidate) => scoreCandidate({
    ...
    mode: args.mode,
    risk: args.risk,
    cost: costs.byPool.get(identityOf(candidate)) ?? costs.byProtocol.get(candidate.protocol) ?? profile(candidate.protocol, { gas: [], executionBps: [], duration: [], cycles: 0 }, 'default'),
    now,
  }))
