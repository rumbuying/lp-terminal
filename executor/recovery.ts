...
export async function inspectRecovery(jobId: string, options?: { reconcile?: boolean }) {
  ...
  if (mintFact?.hash) {
    ...
    mintedTokenId = receiptLiquidityFlows(receipt, job.config.positionManager, 0n).mintedTokenId?.toString()
  }
  ...
  try {
    ...
    const [owner, raw] = await Promise.all([
      publicClient.readContract({ address: job.config.positionManager, abi: clPmAbi, functionName: 'ownerOf', args: [tokenId] }),
      publicClient.readContract({ address: job.config.positionManager, abi: clPmAbi, functionName: 'positions', args: [tokenId] }),
    ])
    activePosition = { owner, liquidity: (raw as readonly unknown[])[7]?.toString(), exists: true }
  } catch {
