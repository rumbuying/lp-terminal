...
/** Human price used by the dashboard: units of quote token per one risk token. */
export function quotePerRiskAtTick(tick: number, config: StrategyConfig, snapshot: StrategyPositionSnapshot): number {
  const token1Per0 = tickToPrice(tick, snapshot.token0Decimals, snapshot.token1Decimals)
  return low(config.quoteToken) === low(snapshot.token1) ? token1Per0 : 1 / token1Per0
}
