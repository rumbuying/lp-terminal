...
import { burnCall, collectCall, decreaseCall, decreaseCollectCall, mintCall, v4DecreaseCall, v4MintCall } from './steps'
...
import { NATIVE, requireGov } from '../src/config/addresses'
...
async function runJob(job: ReturnType<typeof runnableJobs>[number]) {
  ...
  try {
    ...
    const trackedTokens = [...new Set([...walletAllocationTokens(job.walletId), ...tokens.map(low), low(SETTLEMENT), ...(job.config.staking?.enabled ? [low(requireGov().UP)] : [])])] as Address[]
    ...
    if (job.plan.action === 'collect_fees') {
      ...
      if (job.config.fees.handling === 'convert_to_quote') {
        ...
        if (riskAmount > 0n) {
          const tokenIn = routeCurrency(job.config.riskToken)
          const tokenOut = routeCurrency(job.config.quoteToken)
          const quote = await quoteKyber(tokenIn, tokenOut, riskAmount, { protocol: job.config.protocol, tickSpacing: snapshot.tickSpacing, feePpm: snapshot.feePpm })
          await gatedKyberTx({ routeSummary: quote.routeSummary, tokenIn, tokenOut, sender: job.config.owner, recipient: job.config.owner, amountIn: riskAmount, slippageBps: job.config.safeguards.maxSlippageBps, nativeIn: isNativeCurrency(job.config.riskToken) })
    ...
    const decreaseReceipt = await sendTracked({
      ...
      tx: job.config.protocol === 'univ4'
        ? v4DecreaseCall(job.config, beforeDecrease)
        : job.config.execution.lowTransactionMode ? decreaseCollectCall(job.config, beforeDecrease) : decreaseCall(job.config, beforeDecrease),
    })
    receipts.push(decreaseReceipt)
    const v3DecreaseFlows = job.config.protocol === 'univ4'
      ? null
      : receiptLiquidityFlows(decreaseReceipt, job.config.positionManager, BigInt(snapshot.tokenId))
    if (v3DecreaseFlows && v3DecreaseFlows.principal0 === 0n && v3DecreaseFlows.principal1 === 0n) throw new Error('E_RECEIPT_FACT')

    stepIndex = 2
    let collectReceipt = decreaseReceipt
    if (job.config.protocol === 'univ4' || job.config.execution.lowTransactionMode) {
    ...
    const delta0 = job.config.protocol === 'univ4'
      ? receivedByWallet(positionBaseline[low(tokens[0])], afterCollect[low(tokens[0])], decreaseReceipt, tokens[0])
      : afterCollect[low(tokens[0])] - positionBaseline[low(tokens[0])]
    const delta1 = job.config.protocol === 'univ4'
      ? receivedByWallet(positionBaseline[low(tokens[1])], afterCollect[low(tokens[1])], decreaseReceipt, tokens[1])
      : afterCollect[low(tokens[1])] - positionBaseline[low(tokens[1])]
    if (delta0 < 0n || delta1 < 0n) throw new Error('E_UNSUPPORTED_TOKEN')
    const collectFlows = job.config.protocol === 'univ4'
      ? {
          collected0: delta0,
          collected1: delta1,
          // v4 settles principal and fees in one delta. Attribute only the
          // fees proved immediately before signing; any last-block accrual is
          // conservatively left with principal rather than taxing principal.
    ...
    const funds = {
      ...
      fee0: collectFlows.collected0 - collectFlows.principal0,
      fee1: collectFlows.collected1 - collectFlows.principal1,
    }
    ...
    } else if (job.config.protocol === 'univ4' || job.config.execution.lowTransactionMode) {
    ...
    const cycle = await planCycleSwaps({
      ...
      quote: async (tokenIn, tokenOut, amountIn) => {
        const route = await quoteKyber(routeCurrency(tokenIn), routeCurrency(tokenOut), amountIn, { protocol: job.config.protocol, tickSpacing: snapshot.tickSpacing, feePpm: snapshot.feePpm })
        return { amountOut: BigInt(route.routeSummary.amountOut), routeSummary: route.routeSummary }
      },
    })
    ...
    if (job.config.execution.maxDailyTurnoverQuote !== undefined) {
      ...
      const rewardTurnover = rewardQuote * (low(job.config.quoteToken) === low(WRAPPED_NATIVE) ? 1n : 2n)
    ...
    for (const intent of allSwaps) {
      ...
      const routeOut = routeCurrency(intent.tokenOut)
      const nativeIn = isNativeCurrency(intent.tokenIn)
      const approvalQuote = await quoteKyber(routeIn, routeOut, intent.amountIn, fallback)
      const approvalGate = await gatedKyberTx({ routeSummary: approvalQuote.routeSummary, tokenIn: routeIn, tokenOut: routeOut, sender: job.config.owner, recipient: job.config.owner, amountIn: intent.amountIn, slippageBps: job.config.safeguards.maxSlippageBps, nativeIn })
      const approvalReceipts = nativeIn ? [] : await grantStrategyAllowance({ config: job.config, jobId: job.id, stepIndex: 4, txIndexStart: approvalTx, privateKey: unlocked.privateKey, token: intent.tokenIn, spender: approvalGate.approvalTarget, amount: intent.amountIn, forceExact: approvalGate.exactApproval })
      ...
      const spent = spentByWallet(before[low(intent.tokenIn)], after[low(intent.tokenIn)], swapReceipt, intent.tokenIn)
      const gained = receivedByWallet(before[low(intent.tokenOut)], after[low(intent.tokenOut)], swapReceipt, intent.tokenOut)
      const execution = allocateSwapExecution(executableIntent, spent, gained, gated.minOut)
      const revokeReceipts = nativeIn ? [] : await revokeStrategyAllowance({ config: job.config, jobId: job.id, stepIndex: 4, txIndexStart: approvalTx, privateKey: unlocked.privateKey, token: intent.tokenIn, spender: gated.approvalTarget, forceExact: gated.exactApproval })
    ...
    stepIndex = 6
    receipts.push(...await (job.config.protocol === 'univ4'
      ? grantV4CurrencyAllowance({ config: job.config, jobId: job.id, stepIndex, privateKey: unlocked.privateKey, token: snapshot.token0, amount: capital.deploy0 })
      : grantStrategyAllowance({ config: job.config, jobId: job.id, stepIndex, privateKey: unlocked.privateKey, token: snapshot.token0, spender: job.config.positionManager, amount: capital.deploy0 })))
    stepIndex = 7
    receipts.push(...await (job.config.protocol === 'univ4'
      ? grantV4CurrencyAllowance({ config: job.config, jobId: job.id, stepIndex, privateKey: unlocked.privateKey, token: snapshot.token1, amount: capital.deploy1 })
      : grantStrategyAllowance({ config: job.config, jobId: job.id, stepIndex, privateKey: unlocked.privateKey, token: snapshot.token1, spender: job.config.positionManager, amount: capital.deploy1 })))

    stepIndex = 8
    const mintRange = freshRange(job.config, snapshot, mintPool.tick, job.plan.triggerSide, job.plan.rangeScale ?? 1)
    const beforeMint = await readTokenBalances(job.config.owner, tokens)
    const mintReceipt = await sendTracked({
      ...
      privateKey: unlocked.privateKey,
      tx: job.config.protocol === 'univ4'
        ? v4MintCall({ config: job.config, snapshot, tickLower: mintRange.tickLower, tickUpper: mintRange.tickUpper, amount0Desired: capital.deploy0, amount1Desired: capital.deploy1, sqrtPriceX96: mintPool.sqrtPriceX96 })
        : mintCall({ config: job.config, snapshot, tickLower: mintRange.tickLower, tickUpper: mintRange.tickUpper, amount0Desired: capital.deploy0, amount1Desired: capital.deploy1, feePpm: mintPool.feePpm, sqrtPriceX96: mintPool.sqrtPriceX96 }),
    })
    receipts.push(mintReceipt)
    const afterMint = await readTokenBalances(job.config.owner, tokens)
    ...
    const v3MintFlows = job.config.protocol === 'univ4' ? null : receiptLiquidityFlows(mintReceipt, job.config.positionManager, 0n)
    const mintFlows = {
      mintedTokenId,
      minted0: job.config.protocol === 'univ4'
        ? spentByWallet(beforeMint[low(snapshot.token0)], afterMint[low(snapshot.token0)], mintReceipt, snapshot.token0)
        : v3MintFlows!.minted0,
      minted1: job.config.protocol === 'univ4'
        ? spentByWallet(beforeMint[low(snapshot.token1)], afterMint[low(snapshot.token1)], mintReceipt, snapshot.token1)
        : v3MintFlows!.minted1,
    }
    if (!mintFlows.mintedTokenId) throw new Error('E_MINT_TOKEN_ID')
    if (mintFlows.minted0 < 0n || mintFlows.minted1 < 0n || mintFlows.minted0 > capital.deploy0 || mintFlows.minted1 > capital.deploy1)
      throw new Error('E_UNSUPPORTED_TOKEN')
    const newOwner = await publicClient.readContract({ address: job.config.positionManager, abi: job.config.protocol === 'univ4' ? v4PositionManagerAbi : clPmAbi, functionName: 'ownerOf', args: [mintFlows.mintedTokenId] })
    if (low(newOwner) !== low(job.config.owner)) throw new Error('E_OWNER')

    stepIndex = 9
    receipts.push(...await (job.config.protocol === 'univ4'
      ? revokeV4CurrencyAllowance({ config: job.config, jobId: job.id, stepIndex, privateKey: unlocked.privateKey, token: snapshot.token0 })
      : revokeStrategyAllowance({ config: job.config, jobId: job.id, stepIndex, privateKey: unlocked.privateKey, token: snapshot.token0, spender: job.config.positionManager })))
    stepIndex = 10
    receipts.push(...await (job.config.protocol === 'univ4'
      ? revokeV4CurrencyAllowance({ config: job.config, jobId: job.id, stepIndex, privateKey: unlocked.privateKey, token: snapshot.token1 })
      : revokeStrategyAllowance({ config: job.config, jobId: job.id, stepIndex, privateKey: unlocked.privateKey, token: snapshot.token1, spender: job.config.positionManager })))
    ...
    if (job.config.staking?.enabled) {
      ...
      ledger.push({ id: `${job.id}-staking-reward`, strategyId: job.config.id, cycleId: `cycle-${job.id}`, jobId: job.id, ts: now, kind: 'staking_reward', token: requireGov().UP, amount: rewardUp.toString(), txHash: rewardContext?.withdrawTxHash, meta: { convertedToWeth: rewardWeth.toString(), convertedToQuote: rewardQuote.toString(), quoteToken: job.config.quoteToken } })
      if (rewardUp > 0n && rewardContext?.swapTxHash) ledger.push(
        { id: `${job.id}-reward-swap-in`, strategyId: job.config.id, cycleId: `cycle-${job.id}`, jobId: job.id, ts: now, kind: 'swap_in', token: requireGov().UP, amount: rewardUp.toString(), txHash: rewardContext.swapTxHash, meta: { source: 'staking_reward' } },
        { id: `${job.id}-reward-swap-out`, strategyId: job.config.id, cycleId: `cycle-${job.id}`, jobId: job.id, ts: now, kind: 'swap_out', token: WRAPPED_NATIVE, amount: rewardWeth.toString(), txHash: rewardContext.swapTxHash, meta: { source: 'staking_reward', rewardFinal: !hasQuoteHop, quotedOut: rewardQuotedWeth.toString() } },
      )
      if (hasQuoteHop && rewardWeth > 0n && rewardContext?.quoteSwapTxHash) ledger.push(
        { id: `${job.id}-reward-quote-swap-in`, strategyId: job.config.id, cycleId: `cycle-${job.id}`, jobId: job.id, ts: now, kind: 'swap_in', token: WRAPPED_NATIVE, amount: rewardWeth.toString(), txHash: rewardContext.quoteSwapTxHash, meta: { source: 'staking_reward' } },
