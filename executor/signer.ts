import { keccak256, parseUnits, zeroAddress, type Address, type Hex } from 'viem'
...
export async function sendTracked(args: { config: StrategyConfig; jobId: string; stepIndex: number; txIndex?: number; privateKey: Hex; tx: SafeTx }) {
  ...
  if ((await retryPreflightRpc(() => publicClient.getChainId())) !== EXECUTOR.chainId) throw new Error('E_CHAIN')
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    if (args.config.execution.maxGasQuotePerTx) {
      ...
      if (args.config.quoteToken.toLowerCase() !== EXECUTOR.network.wrappedNative.toLowerCase()
        && args.config.quoteToken.toLowerCase() !== zeroAddress) throw new Error('E_LIMIT_PRICE')
