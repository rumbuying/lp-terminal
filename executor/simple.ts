...
export async function startSimpleStrategy(input: unknown, walletId: string) {
  ...
  if (draft.chainId !== EXECUTOR.chainId) throw new Error('E_CHAIN')
  ...
  const wrappedNative = EXECUTOR.network.wrappedNative
  const draftHasWeth = [draft.riskToken, draft.quoteToken].some((token) => token.toLowerCase() === wrappedNative.toLowerCase())
  const quoteToken = draftHasWeth ? wrappedNative : draft.quoteToken
  const riskToken = draftHasWeth
    ? [draft.riskToken, draft.quoteToken].find((token) => token.toLowerCase() !== wrappedNative.toLowerCase())!
    : draft.riskToken
