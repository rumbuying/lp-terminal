...
const { db, insertPool, setTokenPrice, setTvl, upsertState, upsertTokenMeta } = await import('./store');
...
test('bounded TVL refresh does not rewrite an unchanged row', () => {
  const pool = A(0x77);
  insertPool({
    address: pool,
    proto: 'univ2',
  ...
  upsertState(pool, { reserve0: 1n, reserve1: 2n, totalSupply: 3n });
  setTvl(pool, 42, true);
...
test('tvl clamps a runaway pool-priced side to the credible side', () => {
  ...
  assert.deepEqual(tvlOf(10_850_000, false, 387, true), {
    tvl: 774,
    approx: true,
  });
  // both credible: plain sum, exact
  assert.deepEqual(tvlOf(5_000, true, 5_200, true), {
    tvl: 10_200,
    approx: false,
  });
  // pool-priced side under the ratio passes through (honest near-edge pools)
  assert.deepEqual(tvlOf(9_000, true, 600, false), {
    tvl: 9_600,
    approx: false,
  });
  // one side unpriced: 2× the priced side, approximate
  assert.deepEqual(tvlOf(null, false, 1_000, true), {
    tvl: 2_000,
    approx: true,
  });
  ...
  assert.deepEqual(tvlOf(null, false, null, false), {
    tvl: null,
    approx: false,
  });
});
...
test('tvl caps a side at the credible depth that established its price', () => {
  ...
  assert.deepEqual(tvlOf(1e24, false, 1e24, false, 1e5, 1e5), {
    tvl: 200_000,
    approx: true,
  });
  // a side under its cap is untouched
  assert.deepEqual(tvlOf(4_000, false, 4_100, false, 1e5, 1e5), {
    tvl: 8_100,
    approx: false,
  });
  // the cap applies before the ratio test, and the ratio test still wins when
  // it is the tighter bound
  assert.deepEqual(tvlOf(1e9, false, 100, false, 1e6, Infinity), {
    tvl: 200,
    approx: true,
  });
});
...
const units = (x: number, dec: number) =>
  BigInt(Math.round(x * 10 ** Math.min(dec, 15))) * 10n ** BigInt(Math.max(dec - 15, 0));

const priceOf = (addr: string) =>
  (
    db.prepare('SELECT price_usd FROM tokens WHERE address = ?').get(addr) as {
...
test('reprice: a boundary-tick pool with no in-range liquidity cannot mint a price', () => {
  ...
  upsertState(usdgKitty, {
    reserve0: units(400, 6),
    reserve1: units(4e8, 18),
    totalSupply: 1n,
  });
  ...
  upsertState(kittyFake, {
    reserve0: units(1e6, 18),
    reserve1: units(1e3, 18),
    totalSupply: 1n,
  });
  ...
  upsertState(fakeJunk, {
    reserve0: units(1e9, 18),
    reserve1: units(1e6, 18),
    totalSupply: 1n,
  });
...
test('reprice crosses state, quote-token and TVL page boundaries', () => {
  ...
  for (let i = 0; i < count; i++) {
    const token = A(0x10_000 + i);
    const pool = A(0x20_000 + i);
    upsertTokenMeta(token, `T${i}`, 6, true);
    insertPool({
      address: pool,
      proto: 'univ2',
      token0: USDG,
    ...
    upsertState(pool, {
      reserve0: units(1_000, 6),
      reserve1: units(1_000, 6),
      totalSupply: 1n,
    });
  }

  const result = reprice();
  assert.ok(result.priced >= count + 1);
  assert.equal(priceOf(A(0x10_000)), 1);
  assert.equal(priceOf(A(0x10_000 + count - 1)), 1);
  const lastTvl = poolTvl(A(0x20_000 + count - 1));
  assert.equal(lastTvl.tvl_usd, 2_000);
  assert.equal(lastTvl.tvl_approx, 0);
});

test('reprice publication rolls back prices and TVL atomically when commit fails', () => {
  const sentinelPool = A(0x20_000);
  setTokenPrice(KITTY, 123, 999, 'pool');
  db.prepare('UPDATE pool_state SET tvl_usd = 777, tvl_approx = 1 WHERE address = ?').run(sentinelPool);
  db.exec(`
    CREATE TEMP TRIGGER fail_reprice_publish
    BEFORE UPDATE OF price_usd ON main.tokens
    WHEN NEW.address = '${KITTY}' AND NEW.price_src = 'pool'
    BEGIN
      SELECT RAISE(ABORT, 'injected publication failure');
    END;
  `);
  try {
    assert.throws(reprice, /injected publication failure/);
  } finally {
    db.exec('DROP TRIGGER temp.fail_reprice_publish');
  }

  assert.equal(priceOf(KITTY), 123);
  const unchangedTvl = poolTvl(sentinelPool);
  assert.equal(unchangedTvl.tvl_usd, 777);
  assert.equal(unchangedTvl.tvl_approx, 1);
});
