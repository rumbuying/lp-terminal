...
import { ADDR, UNI, requireGov } from '../src/config/addresses'
...
function classify(to: Address, data: Hex) {
  try {
    ...
    if (low(to) === low(token0) || low(to) === low(token1) || low(to) === low(requireGov().UP) || low(to) === low(ADDR.STABLE)) return decodeFunctionData({ abi: erc20Abi, data })
...
async function acceptRawTransaction(raw: Hex): Promise<{ hash?: Hex; error?: string }> {
  ...
  if (call.functionName === 'multicall' && (low(to) === low(ADDR.CL_PM) || low(to) === low(UNI.V3_NPM))) {
    ...
    addBalance(requireGov().UP, owner, reward)
    if (reward) logs.push(transferLog(requireGov().UP, gauge, owner, reward, hash, blockHash, blockNumber, logs.length))
...
function callResult(to: Address, data: Hex): { result?: Hex; error?: string } {
  try {
    ...
    if (low(to) === low(requireGov().VOTER)) {
    ...
    if (low(to) === low(token0) || low(to) === low(token1) || low(to) === low(requireGov().UP) || low(to) === low(ADDR.STABLE)) {
      const call = decodeFunctionData({ abi: erc20Abi, data })
      if (call.functionName === 'decimals') return { result: encodeFunctionResult({ abi: erc20Abi, functionName: 'decimals', result: low(to) === low(ADDR.STABLE) ? 6 : 18 }) }
...
try {
  ...
  const runStakedScenario = async (suffix: string, tokenId: bigint, failure?: typeof failOnce | 'pause_after_multicall', expectedRecovery = 'resume_staking_exit', lowTransactionMode = false, rewardAmount = 500n) => {
    ...
    const config = { ...base, execution: { ...base.execution, lowTransactionMode }, staking: { enabled: true, gauge, rewardToken: requireGov().UP, rewardQuoteToken: ADDR.WNATIVE, rewardHandling: 'convert_to_quote_hold' as const } }
    ...
    if (lowTransactionMode) {
      ...
      assert.ok(allowance(requireGov().UP, owner, router) > 0n)
  ...
  const rejectedStake = { ...rejectedStakeBase, staking: { enabled: true, gauge, rewardToken: requireGov().UP, rewardQuoteToken: ADDR.WNATIVE, rewardHandling: 'convert_to_quote_hold' as const } }
  ...
  const usdgBeforeTax = balance(ADDR.STABLE, owner)
  const taxConfig = await runScenario('fee-tax', 34n, undefined, 101, 'univ3', false, true, 2_000_000n)
  assert.equal(balance(ADDR.STABLE, owner) - usdgBeforeTax, 200_000n)
  const taxLedger = store.db.prepare(`SELECT amount,meta_json FROM ledger_entries WHERE strategy_id=? AND kind='swap_out' AND lower(token)=? ORDER BY ts DESC LIMIT 1`).get(
    'strategy-integration-fee-tax', low(ADDR.STABLE),
  ...
  const usdgBeforeHoldTax = balance(ADDR.STABLE, owner)
  store.upsertStrategy(holdTax)
  await monitorOnce({ ignoreSchedule: true })
  await runOnce()
  await waitForState(holdTax.id, 'paused_hold_quote')
  assert.equal(balance(ADDR.STABLE, owner) - usdgBeforeHoldTax, 200_000n)
