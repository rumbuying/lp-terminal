...
try {
  ...
  assert.equal(convertedFee.swaps[0].tokenOut.toLowerCase(), ADDR.USDG.toLowerCase())
  ...
  const stableFundedTax = await planFeeTax({
    ...
    quote: async (tokenIn, tokenOut, amountIn) => ({
      ...
      amountOut: tokenIn.toLowerCase() === ADDR.WETH.toLowerCase() ? amountIn * 2_000n : amountIn * 1_000n,
      routeSummary: { tokenIn, tokenOut, amountIn: amountIn.toString(), amountOut: (tokenIn.toLowerCase() === ADDR.WETH.toLowerCase() ? amountIn * 2_000n : amountIn * 1_000n).toString(), route: [] },
    }),
  })
  assert.equal(stableFundedTax.applied, true)
  assert.equal(stableFundedTax.tax0, 0n)
  assert.ok(stableFundedTax.tax1 > 0n)
  assert.equal(stableFundedTax.swaps[0].tokenIn.toLowerCase(), ADDR.WETH.toLowerCase())
