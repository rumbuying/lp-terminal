...
export function PnlCalendarTab() {
  ...
  const load = async (key = month) => {
    ...
    try {
      ...
      if (ownerRef.current === requestedOwner) setError(cause instanceof Error ? cause.message : 'calendar unavailable')
    }
  ...
  return <div className="panel">
    <div className="panel-head"><div><div className="section-title">{t('pnlCalendar.title')}</div><div className="dim mono-sm">{t('pnlCalendar.subtitle')}</div></div></div>
    ...
    <div className="pnl-calendar">
      ...
      {cells.map((date, index) => {
        ...
        const dayRows = byDate.get(date) ?? [], dayTotals = totals(dayRows), net = dayTotals.reduce((sum, item) => sum + (item.raw > 0n ? 1 : item.raw < 0n ? -1 : 0), 0)
    ...
    {!selectedRows.length ? <div className="dim">{t('pnlCalendar.noData')}</div> : selectedRows.map((row) => <div className="card" key={row.strategyId}>
      <div className="card-head"><span className="card-title">{row.name}</span><strong className={row.pnlRaw !== null && BigInt(row.pnlRaw) >= 0n ? 'green' : 'red'}>{row.pnlRaw === null ? '—' : signedAmount(BigInt(row.pnlRaw), row.quote.symbol, row.quote.decimals)}</strong></div>
