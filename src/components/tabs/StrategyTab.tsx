...
const savedExecutorAdminToken = () => {
  ...
  try { return window.localStorage.getItem(EXECUTOR_ADMIN_TOKEN_KEY)?.trim() ?? '' } catch { return '' }
}
...
export function StrategyTab() {
  ...
  const positions = usePositions(user, { refetchInterval: 120_000, poolsRefetchInterval: 120_000 })
  const pools = usePools({ refetchInterval: 120_000 })
  ...
  const eligible = positions.data?.cl.filter((p) => p.liquidity > 0n && (!p.staked || (p.pool.protocol === 'up33' && !!p.pool.gauge && p.pool.gaugeAlive))) ?? []
  ...
  const refreshExecutor = async (token = accessToken) => {
    const [walletData, strategyData, recoveryData, performanceData] = await Promise.all([
  ...
  const connectExecutor = async (token = accessToken, role: 'wallet' | 'admin' = 'admin') => {
    ...
    try {
      ...
      if (role === 'admin' && normalizedToken) {
        try { window.localStorage.setItem(EXECUTOR_ADMIN_TOKEN_KEY, normalizedToken) } catch { /* storage can be unavailable */ }
      }
      setAuthRole(role)
      ...
      if (role === 'admin' && normalizedToken && normalizedToken === savedExecutorAdminToken()) {
        try { window.localStorage.removeItem(EXECUTOR_ADMIN_TOKEN_KEY) } catch { /* storage can be unavailable */ }
        setAdminToken('')
      }
  ...
  useEffect(() => {
    ...
    const refreshPerformance = () => {
      if (document.hidden) return
      void executorPerformance(accessToken).then((data) => setExecutorPerformanceList(data.strategies)).catch(() => undefined)
    }
  ...
  const addOriginal = (p: (typeof eligible)[number]) => {
    ...
    const draft = originalStrategyDraft({
      ...
      pool: p.pool.address,
      positionManager: p.pool.protocol === 'up33' ? ADDR.CL_PM : UNI.V3_NPM,
      activeTokenId: p.tokenId.toString(),
      riskToken: token0IsWeth ? p.pool.token1 : p.pool.token0,
      quoteToken: token0IsWeth ? p.pool.token0 : p.pool.token1,
