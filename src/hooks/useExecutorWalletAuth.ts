...
import { executorWalletChallenge, executorWalletSessionStorageKey, executorWalletVerify } from '../lib/executorClient'
...
function savedSession(address: Address): StoredSession | undefined {
  try {
    const storageKey = executorWalletSessionStorageKey(address)
    const parsed = JSON.parse(sessionStorage.getItem(storageKey) ?? 'null') as StoredSession | null
...
export function useExecutorWalletAuth(address: Address | undefined, enabled = true, auto = true) {
  ...
  const authenticate = useCallback(async () => {
    ...
    try {
      ...
      sessionStorage.setItem(executorWalletSessionStorageKey(address), JSON.stringify(stored))
  ...
  const invalidate = useCallback(() => {
    if (address) {
      try { sessionStorage.removeItem(executorWalletSessionStorageKey(address)) } catch { /* unavailable storage */ }
    }
    setToken('')
    setStatus('idle')
    attemptedAddress.current = ''
