...
test('CL valuation prefers two direct indexer marks', () => {
  const price = clTokenUsd(oneToOneCl, 18, 18, undefined, undefined, {
    [TOKEN0]: 2,
    [TOKEN1]: 3,
  })
  assert.deepEqual(price && { p0: price.p0, p1: price.p1 }, { p0: 2, p1: 3 })
})
...
test('CL valuation derives the missing side from pool spot and one direct mark', () => {
  ...
  assert.deepEqual(price && { p0: price.p0, p1: price.p1 }, { p0: 2, p1: 2 })
})
