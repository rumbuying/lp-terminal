...
// This covers the Robinhood-only ve(3,3) reward route. Keep it in the shared
// suite, but do not make the BSC build manufacture governance contracts that
// do not exist on that network.
const GOV = CHAIN.gov ?? { UP: '0x0000000000000000000000000000000000000001' as Address }
    ...
    publicClient: {
      readContract: async ({ address, functionName, args }: { address: Address; functionName: string; args?: readonly unknown[] }) => {
        ...
        if (same(address, ADDR.CL_FACTORY) && functionName === 'getPool') {
          ...
          return pair(tokenIn, tokenOut, GOV.UP, ADDR.WNATIVE) ? upWethPool : zeroAddress
        }
        if (same(address, UNI.V3_FACTORY) && functionName === 'getPool') {
          const [tokenIn, tokenOut, fee] = args as readonly [Address, Address, number]
          if (!pair(tokenIn, tokenOut, ADDR.WNATIVE, quoteToken)) return zeroAddress
...
test('two-hop reward quote falls back to canonical native pools and chooses the best V3 fee tier', { skip: !CHAIN.gov }, async () => {
