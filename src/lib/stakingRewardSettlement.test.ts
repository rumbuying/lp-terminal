...
const GOV = CHAIN.gov ?? { UP: '0x0000000000000000000000000000000000000006' as Address }
...
const balances: Record<string, bigint> = {
  [GOV.UP.toLowerCase()]: 0n,
  [ADDR.WNATIVE.toLowerCase()]: 0n,
...
mock.module('../../executor/signer', {
  namedExports: {
    sendTracked: async ({ stepIndex, txIndex = 0 }: { stepIndex: number; txIndex?: number }) => {
      ...
      if (stepIndex === 12) {
        unstaked = true
        balances[GOV.UP.toLowerCase()] = 100n
        value = receipt(1)
        deltas.set(value.transactionHash, { [GOV.UP.toLowerCase()]: 100n })
      } else if (stepIndex === 14 && txIndex === 0) {
        balances[GOV.UP.toLowerCase()] = 0n
        balances[ADDR.WNATIVE.toLowerCase()] = 100n
        value = receipt(2)
        deltas.set(value.transactionHash, { [GOV.UP.toLowerCase()]: -100n, [ADDR.WNATIVE.toLowerCase()]: 100n })
      } else if (stepIndex === 14 && txIndex === 1) {
        balances[ADDR.WNATIVE.toLowerCase()] = 0n
        balances[quoteToken.toLowerCase()] = 90n
        value = receipt(3)
        deltas.set(value.transactionHash, { [ADDR.WNATIVE.toLowerCase()]: -100n, [quoteToken.toLowerCase()]: 90n })
...
test('settles a staking reward through WETH into a non-WETH strategy quote token', { skip: !CHAIN.gov }, async () => {
  const job = {
    id: 'job-two-hop',
    config: {
      owner,
